...
function skipLecture(class_cd, classday, classhour_start_time, class_prog_cd, classroom_no){
	if(class_prog_cd == "G018C003") {
		if(confirm("이미 휴강신청이 되어있습니다.\n휴강신청을 취소하시겠습니까?")) {
			cancelSkipLecture(class_cd, classday, classhour_start_time);
		}		
	} else {
...
function cancelSkipLecture(class_cd, classday, classhour_start_time) {
	var postString = {class_cd: class_cd, classday: classday, classhour_start_time: classhour_start_time};

	$.ajax({
		type: "POST",

		url: getContextPath()+"/prof/class/class_off_cancel",
