...
function certLecture(class_cd, classday, classhour_start_time, attend_sts_cd){
	if(attend_sts_cd=='G023C001'){
		$("#dialog_cert_lecture1").load(getContextPath()+'/student/attend/attend_cert',{class_cd:class_cd, classday:classday, classhour_start_time:classhour_start_time});
		$("#dialog_cert_lecture1").dialog("open");
	}else{
		$("#dialog_cert_lecture2").load(getContextPath()+'/student/attend/attend_cert_confirm',{class_cd:class_cd, classday:classday, classhour_start_time:classhour_start_time});
	  ...
	  $( "#dialog_cert_lecture1" ).dialog({
	    ...
	    buttons: {
	      "확인": function() {
	        ...
	        $("#dialog_cert_lecture2").load(getContextPath()+'/student/attend/attend_cert_confirm',{class_cd:class_cd, classday:classday, classhour_start_time:classhour_start_time, cert_code:cert_code});
	        $('#dialog_cert_lecture2').dialog('open');
	      },
	      "취소": function() {
	        $( this ).dialog( "close" );
