...
<div id="board">
		...
		<table width="700" border="0" cellspacing="0" cellpadding="0">
			...
			<thead>
				<tr>
			        ...
			        <th width="40" scope="col">사번</td>
			        <th width="55" scope="col">교수명</td>
			        <th width="45" scope="col">전화번호</td>
			        <th width="35" scope="col">정상강의수</td>
			        <th width="35" scope="col">처리강의수</td>
			        <th width="35" scope="col">미 처리수</td>
			        <th width="30" scope="col">사용률</td>
				</tr>
			</thead>
			<tbody>
			<c:choose>
				...
				<c:otherwise>
					<c:forEach var="b" items="${pb.list}" varStatus="status">
						<tr class="tr_over">
							<td>${b.row_no}</td>
							<td><a href="${pageContext.request.contextPath }/muniv/prof/prof_view?prof_no=${l.PROF_NO}">${l.PROF_NO }</a></td>
							<td>${b.HP_NO }</td>
							<td>${b.ALLCNT }</td>
						<c:if test="${list.PROCCNT==0}">  
						    <td style="color:red;">${l.PROCCNT }</td>
						</c:if>
						<c:if test="${list.PROCCNT!=0}">  
						    <td>${l.PROCCNT }</td>
						</c:if>
	...
	<div class="btn_area">
		<a href="${pageContext.request.contextPath }/excel?target=attendStatus">
