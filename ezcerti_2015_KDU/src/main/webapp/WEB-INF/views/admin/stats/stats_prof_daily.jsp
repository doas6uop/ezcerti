...
<div id="contents">
<!-- 관리자출결현황 -->
<c:set var="lecture" value="${statsProfDaily[0] }"/>	
<div class="titlebg">
...
<c:forEach var="list" items="${statsProfDaily }">
  <tr>
    <td>${list.row_no }</td>
    <td>${list.class_type_name }</td>
    <td>${list.class_sts_name }</td>
    <td><a href="${pageContext.request.contextPath }/muniv/stats/stats_prof_dailyattendee?prof_no=${param.prof_no}&subject_cd=${lecture.subject_cd}&subject_div_cd=${lecture.subject_div_cd}&classday=${list.classday}&classhour_start_time=${list.classhour_start_time}">${list.classday} (${list.classday_name })</a></td>
    <td>${list.classhour_start_time} ~ ${list.classhour_end_time }</td>
<c:choose>
	<c:when test="${list.class_type_cd=='G019C002' }">
		<td colspan="5">${list.class_type_name }</td>
	</c:when>
	<c:otherwise>
