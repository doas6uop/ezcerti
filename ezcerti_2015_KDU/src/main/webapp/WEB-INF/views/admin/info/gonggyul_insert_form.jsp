...
$(function() {
	 ...
	 $.ajax({
	        ...
	        success: function(data) {
	        	...
	        	$.each(data.list, function(key, value){
	        		tmpText += "<p><input type='checkbox' class='checkbox_align' name='chk_classname' value=" + value.class_cd + "_" + value.classhour_start_time + "> " + value.class_name 
 ...
 function sendAjax() {
    ...
    $.ajax({
        ...
        url: "${pageContext.request.contextPath}/muniv/info/gonggyul_insert_ok",
        data: frm,
        dataType : 'text',
        processData : false,
        contentType : false,
...
function doPopupSearchStudent() {
	
	var targetURL = "${pageContext.request.contextPath}/muniv/info/gonggyul_search_popup";
	
	window.open(targetURL,'gongGyulPop','width=710, height=630, top=0, left=250, toolbar=no, menubar=no, scrollbars=yes');
...
function doClearReceiver() {
	 $("#student_no").val("");
	 $("#student_name").val("");
	 $("#to_user_name_span").text("");
	 $("#sdate").val("");
	 $("#edate").val("");
	 $('#submit_date').val("");
	 $("#class_name_box").html("");
	 $("#class_name_box").text("공결처리대상을 지정해주세요.");
}
...
<div id="board">
	<form id="boardFrm" enctype="multipart/form-data" onsubmit="javascript:sendAjax(); return false;">
		...
		<div id="board_write">
			...
			<table border="0" cellpadding="0" cellspacing="0" summary="글쓰기">
				...
				<tr>
					<th>공결대상 학생</th>
					<td>
						<div style="padding: 4px; border: 1px solid #7f9db9; border-image: none; width: 65%; height: auto; min-height : 14px; float: left; margin-top: 4px;">
							<span id="to_user_name_span"></span>
							<input type="hidden" name="student_name" id="student_name" size="20" value="" class="float_left mg_r5" readonly />
							<input type="hidden" name="student_no" id="student_no" size="20"  value="" class="float_left" readonly />
						</div>
						<div style="padding: 3px 4px 4px; width: 30%; text-align: center; float: left;">
							<button id="search_student_btn" class="btn-black" type="button" onclick="doPopupSearchStudent()">대상검색</button>
							<button type="button" class="btn-black" style="margin-left: 3px;" onclick="doClearReceiver()">초기화</button>
						</div>
					</td>
				</tr>
				<tr>
					...
					<td>
						<c:choose>
							<c:when test="${term_cd eq 'G002C001'}">
								${year}년 1학기
							</c:when>
							<c:otherwise>
								${year}년 2학기
				...
				<tr>
					<th>기간</th>
					<td>
						<input type="text" id="sdate" name="gong_ilja_start" class="searchtextbox" value="${sdate }" style="width:120px;" readonly="readonly"> ~ <input type="text" id="edate" name="gong_ilja_end" value="${edate }" class="searchtextbox" style="width:120px;" readonly="readonly">
					</td>
				</tr>
				<tr>
					<th>과목</th>
					<td id="class_name_box">공결처리대상을 지정해주세요.</td>
				</tr>
				<tr>
					<th>제출일자</th>
					<td id="class_name_box"><input type="text" id="submit_date" name="submit_date" class="searchtextbox" value="${submit_date }" style="width:120px;" readonly="readonly"></td>
				</tr>
				<tr>
					<th>사유</th>
					<td>
						<textarea id="gong_sayu" name="gong_sayu" class="hanText" rows="5" cols="10" style="border: 1px solid #7f9db9;" ></textarea>
					</td>
				</tr>
			</table>
		</div>
		<div class="btn_area">
			...
			<a href="#" onclick="javascript:sendAjax()"><img src="${pageContext.request.contextPath}/resources/images/admin/insert_btn.gif" alt="등록" /></a>
		</div>
