...
<tr>
	...
	<c:forEach var="list" items="${academicProfClassStats}" begin="0" varStatus="status" end="0">
		<c:forEach var="idx" begin="1" varStatus="status" end="${academicProfClassCount }">
			...
			<th>${fn:replace(fn:replace(list[day], '-(', '<br/>(') , '-', '<br/>')}</th>
		</c:forEach>
	</c:forEach>
...
<c:forEach var="list" items="${academicProfClassStats}" begin="1" varStatus="status" end="${fn:length(academicProfClassStats) - 1}">
	...
	<c:set var="varDeptName" value="${list.DEPT_NAME}" />
	
	<c:if test="${fn:length(varDeptName) > 3}">
		<c:set var="varDeptName" value="${fn:substring(varDeptName, 0, 3)}" />
	</c:if>
	
	<tr>
			<td>${list.RNUM }</td>
			<%-- <td style="width:100px">${list.DEPT_NAME }</td> --%>
			<td style="width:100px">${varDeptName }</td>
			...
			<c:forEach var="idx" begin="1" varStatus="status" end="${academicProfClassCount }">
				...
				<td>
					<c:choose>
						<c:when test="${list[day] eq '출석'}">O</c:when>
						<c:when test="${list[day] eq '공결' or list[day] eq '2'}">出</c:when>
						<c:when test="${list[day] eq '결석'}"><span style="font-weight: bold; color:red">/</span></c:when>
						<c:when test="${list[day] eq '지각'}"><span style="font-weight: bold; color:red">X</span></c:when>
						<c:when test="${list[day] eq '휴학'}">(휴)</c:when>
						<c:when test="${list[day] eq '제적'}">(제)</c:when>
						<c:when test="${list[day] eq '0'}"></c:when>
						<c:when test="${list[day] eq '출결전'}"></c:when>
						<c:otherwise>
							<c:choose>
								<c:when test="${fn:length(list[day]) > 2}">
									<span style="font-size: 10px">(휴)<br>${fn:replace(fn:substring(list[day], 5, fn:length(list[day])), '-', '<br/>')}</span>	
								</c:when>
								<c:otherwise>
									${list[day]}
								</c:otherwise>
